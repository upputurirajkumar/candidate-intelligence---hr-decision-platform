  ...
  return (
    <div id="leaderboard-view" className="space-y-6">
      ...
      <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-xs flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2">
                ...
                <span className="text-xs bg-indigo-100 text-indigo-800 px-2.5 py-0.5 rounded-full font-mono font-semibold">
                  {filteredRanked.length} Candidates
                </span>
              </h2>
              <p className="text-xs text-slate-500">
                Job Requisition: <span className="font-semibold text-slate-800">{job.title}</span> ({job.department})
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 text-[11px] bg-slate-50 border border-slate-200 p-2.5 rounded-xl text-slate-600 font-mono">
      ...
      <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-xs space-y-3">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-2.5 text-xs">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-9 pr-3 py-2 text-xs text-slate-800 placeholder-slate-400 focus:outline-hidden focus:ring-1 focus:ring-indigo-500"
            />
          </div>
          ...
          <div>
            <select
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-700 focus:outline-hidden focus:ring-1 focus:ring-indigo-500"
            >
        ...
        {selectedCandidateIds.length > 0 && (
          <div className="flex flex-wrap items-center justify-between gap-3 bg-indigo-50 border border-indigo-100 p-3 rounded-xl animate-in fade-in duration-150">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-1.5 bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-1.5 rounded-lg text-xs font-semibold shadow-xs cursor-pointer"
              >
              ...
              <button
                ...
                disabled={isBulkProcessing}
                className="flex items-center gap-1.5 bg-slate-700 hover:bg-slate-800 text-white px-3 py-1.5 rounded-lg text-xs font-semibold cursor-pointer"
              >
          ...
          filteredRanked.map((cand, idx) => {
            ...
            return (
              <div
                ...
                className={`bg-white rounded-2xl p-5 border transition-all hover:shadow-md cursor-pointer relative group ${
                  isTop3 
              ...
              >
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-600 font-medium mt-0.5">
                          {cand.currentRole} • <span className="text-slate-500">{cand.currentCompany}</span>
                        </p>
                  ...
                  <div className="flex flex-wrap items-center gap-3 lg:justify-end">
                    ...
                    <div className="bg-indigo-50 border border-indigo-100 rounded-xl px-4 py-2 text-center">
                      <div className="text-[10px] uppercase font-bold text-indigo-700">Dynamic Match</div>
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 bg-slate-100 hover:bg-indigo-50 hover:text-indigo-600 text-slate-700 border border-slate-200 px-3 py-2 rounded-xl text-xs font-semibold transition-all cursor-pointer"
                    >
                ...
                <div className="flex flex-wrap gap-1.5 mt-3 pt-3 border-t border-slate-100">
                  <span className="text-[10px] uppercase font-bold text-slate-400 self-center mr-1">Skills:</span>
                  {(cand.skills || []).slice(0, 6).map((s, sIdx) => {
                    ...
                    return (
                      ...
                      >
                        {isReq && <CheckCircle2 className="w-2.5 h-2.5 text-indigo-600" />}
                        {sName}
                      </span>
                    );
                  })}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                    ...
                    <span className="text-xs bg-indigo-100 text-indigo-800 px-2 py-0.5 rounded-full font-mono">
                      Ranked Match: {explainingCandidate.overallFitScore}%
                    </span>
                  </h3>
                  <p className="text-xs text-slate-500">
                    Comprehensive explainability for role: <span className="font-semibold text-slate-800">{job.title}</span>
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="space-y-4 text-xs">
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-2">
                ...
                <p className="text-slate-600 leading-relaxed">
                  ...
                  <strong>{explainingCandidate.currentCompany}</strong>. Historical tenure and leadership responsibilities align with the senior requirements of {job.title}.
                </p>
              </div>
            ...
            <div className="flex items-center justify-between pt-3 border-t border-slate-100">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="flex items-center gap-1.5 px-4 py-2 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 border border-indigo-200 rounded-lg text-xs font-semibold cursor-pointer"
                >
            ...
            <div className="space-y-2">
              {[
                ...
                <button
                  ...
                  className="w-full text-left px-3.5 py-2 rounded-xl border border-slate-200 hover:border-indigo-500 hover:bg-indigo-50 text-xs font-semibold text-slate-800 transition-all flex items-center justify-between cursor-pointer"
                >
                  <span>{stage}</span>
